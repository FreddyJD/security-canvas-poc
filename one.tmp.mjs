...
const server = http.createServer(async (req, res) => {
	...
	// One-click sign-in: opens the browser, waits for the redirect, loads data.
	...
	actions: [
		...
		},
		{
			...
			handler: async () => {
				...
				await signIn();
				await refresh();
				return { signedIn: true, count: state.assessments.length };
			},
		},
		{
			name: "refresh_queue",
...
<script>
  ...
  function gateHtml(s) {
    ...
    return shield +
      ...
      '<button class="primary" id="gate-btn">Sign in with Microsoft</button>';
  }
